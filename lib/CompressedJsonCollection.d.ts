...
export default class CompressedJsonCollection<T> extends EventEmitter {
    ...
    readonly cleanCompressedJson: ICompressedJsonCollectionState;
    constructor(definition: ICompressedJsonCollectionDefinition<T>, items?: T[]);
    add(items: T | T[]): void;
    remove(items: T | T[]): void;
    removeByIndex(index: number): void;
    removeByIndex(from: number, to: number): void;
    clear(): void;
    ...
    private internalRemoveByIndex(index);
    private internalRemoveIndexRange(from, to);
    private internalAdd;
