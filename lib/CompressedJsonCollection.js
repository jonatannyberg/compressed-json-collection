...
var CompressedJsonCollection = (function () {
    ...
    CompressedJsonCollection.prototype.add = function (items) {
        ...
        itemsToAdd.forEach(function (item) { return add(item); });
    };
    ...
    CompressedJsonCollection.prototype.removeByIndex = function (index, to) {
        if (to != null && index > to)
        ...
        if (to == null) {
            ...
            var idxToRemove = index;
            var state = this._state;
            var removedItems = this.items.splice(idxToRemove, to - idxToRemove + 1);
            ...
            for (var _b = 0, _c = state.definitions.filter(function (d) { return d.definition.encoding === 0 /* RUNLENGTH */; }); _b < _c.length; _b++) {
                ...
                var currRlItemIdx = 0;
                while (true) {
                    var item = rlItems[currRlItemIdx];
                ...
                var itemCountToRemove = removedItems.length;
                var currRlItem = rlItems[currRlItemIdx];
                ...
                while (itemCountToRemove > 0) {
                    var currRlItem_1 = rlItems[currRlItemIdx];
                    if (typeof currRlItem_1 === 'object') {
                        var item = currRlItem_1;
