...
const CompressedCollection_1 = require('../src/CompressedCollection');
describe('CompressedCollection basics', () => {
    describe('diff encoding with single parameter', () => {
        it('single item', done => {
            const data = [{ id: 0 }];
            const definition = { properties: { id: { encoding: 1 /* DIFF */ } } };
            const collection = new CompressedCollection_1.default(definition, data);
            Should.equal(collection.items.length, data.length, 'Collection contains wrong number of items.');
            const decompressedItems = CompressedCollection_1.default.decompress(collection.compressedJson);
    ...
    describe('runlength encoding', () => {
        it('single item', done => {
            const data = [{ state: 'OK' }];
            const definition = { properties: { state: { encoding: 0 /* RUNLENGTH */ } } };
            const collection = new CompressedCollection_1.default(definition, data);
            Should.equal(collection.items.length, data.length, 'Collection contains wrong number of items.');
            const decompressedItems = CompressedCollection_1.default.decompress(collection.compressedJson);
            Should.deepEqual(decompressedItems, data, 'Decompressed data does not match source data.');
    ...
    describe('raw encoding', () => {
        it('single item', done => {
            ...
            const definition = { properties: { something: { encoding: 2 /* RAW */ } } };
            const collection = new CompressedCollection_1.default(definition, data);
            Should.equal(collection.items.length, data.length, 'Collection contains wrong number of items.');
            const decompressedItems = CompressedCollection_1.default.decompress(collection.compressedJson);
            Should.deepEqual(decompressedItems, data, 'Decompressed data does not match source data.');
...
describe('CompressedCollection inserts with sort', () => {
    describe('encoding with sorting, basics', () => {
        it('empty collection, add one item.', done => {
            const sort = (a, b) => a.val < b.val ? -1 : a.val > b.val ? 1 : 0;
            const data = [];
            const definition = {
                sort: sort,
                properties: {
...
describe('CompressedCollection querying', () => {
    it('querying collection, test one', done => {
        ...
        const decompressedItems = CompressedCollection_1.default.decompress(result);
    ...
    describe('basic filter', () => {
        describe('unsorted inserting', () => {
            it('empty collection with basic filtertype, add one item that should not be filtered', done => {
                const data = [];
                const definition = {
                    insertionHandler: (item, index, array) => item.val < 4,
                    properties: {
                        val: { encoding: 1 /* DIFF */, decimalDigits: 0 },
                        rl: { encoding: 0 /* RUNLENGTH */ }
                    }
            ...
            it('initiating collection filtered with basic filtertype with data, some items should be filtered away', done => {
                let data = [{ val: 0, rl: 'OK' }, { val: 1, rl: 'OK' }, { val: 2, rl: 'OK' }, { val: 3, rl: 'OK' }, { val: 4, rl: 'OK' }];
                const definition = {
                    insertionHandler: (item, index, array) => item.val > 1,
                    properties: {
                        val: { encoding: 1 /* DIFF */, decimalDigits: 0 },
                        rl: { encoding: 0 /* RUNLENGTH */ }
                    }
    ...
    describe('advanced filter', () => {
        describe('unsorted inserting', () => {
            it('empty collection with basic filtertype, add one item that should not be filtered', done => {
                const data = [];
                const definition = {
                    insertionHandler: new (class {
                        insert(items, collection) {
        ...
        describe('advanced usage tests', () => {
            ...
            it('handler that alters collection', done => {
                ...
                for (const item of moreData)
                    collection.add(item);
                Should.deepEqual(CompressedCollection_1.default.decompress(collection.compressedJson), [{ val: 2, rl: 'OK', insertIdx: 0 }, { val: 10, rl: 'OK', insertIdx: 4 }, { val: 18, rl: 'OK', insertIdx: 8 }, { val: 26, rl: 'OK', insertIdx: 12 }, { val: 28, rl: 'OK', insertIdx: 13 }, { val: 30, rl: 'OK', insertIdx: 14 }], 'Decompressed result does not match expectation.');
                done();
            });
            ...
            it('real advanced gps usecase (time baser insert filtering)', done => {
                const InsertionHandler = class {
                    constructor() {
                        this.buffer = [];
                        this.isAlteringInternal = false;
                        this.compactedMin = new Date(Date.now() + 31536000000);
                ...
                ;
                const sort = (a, b) => a.time < b.time ? -1 : a.time > b.time ? 1 : 0;
                const data = gpsTestData.slice(1, 10);
                const definition = {
                    sort: sort,
                    insertionHandler: new InsertionHandler(),
                    properties: {
