...
var CompressedJsonCollection_1 = require("../CompressedJsonCollection");
describe('CompressedCollection basics', function () {
    describe('diff encoding with single parameter', function () {
        it('single item', function (done) {
            var data = [{ id: 0 }];
            var definition = { properties: { id: { encoding: 1 /* DIFF */ } } };
            var collection = new CompressedJsonCollection_1.default(definition, data);
            Should.equal(collection.items.length, data.length, 'Collection contains wrong number of items.');
            var decompressedItems = CompressedJsonCollection_1.default.decompress(collection.compressedJson);
        ...
        it('decreasing item values', function (done) {
            var data = [{ id: 3 }, { id: 2 }, { id: 1 }, { id: 0 }];
            var definition = { properties: { id: { encoding: 1 /* DIFF */ } } };
            var collection = new CompressedJsonCollection_1.default(definition, data);
            Should.equal(collection.items.length, data.length, 'Collection contains wrong number of items.');
            var decompressedItems = CompressedJsonCollection_1.default.decompress(collection.compressedJson);
    ...
    describe('basic diff encoding with multiple parameters', function () {
        it('basic diff encoding with two parameters', function (done) {
            var data = [
            ...
            var definition = {
                properties: {
                    id: { encoding: 1 /* DIFF */ },
                    parameterOne: { encoding: 1 /* DIFF */, decimalDigits: 4 }
                }
            };
            var collection = new CompressedJsonCollection_1.default(definition, data);
            Should.equal(collection.items.length, data.length, 'Collection contains wrong number of items.');
        ...
        it('basic diff encoding with two parameters, one using the decimalDigits', function (done) {
            var data = [
        ...
        it('basic diff encoding with two parameters, one using the decimalDigits and factor', function (done) {
            var data = [
        ...
        it('diff encoding more data', function (done) {
            var data = [];
    ...
    describe('runlength encoding', function () {
        it('single item', function (done) {
            var data = [{ state: 'OK' }];
            var definition = { properties: { state: { encoding: 0 /* RUNLENGTH */ } } };
            var collection = new CompressedJsonCollection_1.default(definition, data);
            Should.equal(collection.items.length, data.length, 'Collection contains wrong number of items.');
            var decompressedItems = CompressedJsonCollection_1.default.decompress(collection.compressedJson);
            Should.deepEqual(decompressedItems, data, 'Decompressed data does not match source data.');
        ...
        it('multiple varying items', function (done) {
            var data = [
                { state: 'OK' },
                { state: 'OK' },
                { state: 'ARCHIVED' },
        ...
        it('run length encoding more data', function (done) {
            var data = [];
            for (var i = 0; i < 10000; i++)
                data.push({
            ...
            var definition = {
                properties: {
                    state: { encoding: 0 /* RUNLENGTH */ },
                    safe: { encoding: 0 /* RUNLENGTH */ },
                    count: { encoding: 0 /* RUNLENGTH */ },
                }
            };
            var collection = new CompressedJsonCollection_1.default(definition, data);
            Should.equal(collection.items.length, data.length, 'Collection contains wrong number of items.');
    ...
    describe('raw encoding', function () {
        it('single item', function (done) {
            var data = [{ something: { name: 'John', id: 1234 } }];
            var definition = { properties: { something: { encoding: 2 /* RAW */ } } };
            var collection = new CompressedJsonCollection_1.default(definition, data);
            Should.equal(collection.items.length, data.length, 'Collection contains wrong number of items.');
            var decompressedItems = CompressedJsonCollection_1.default.decompress(collection.compressedJson);
            Should.deepEqual(decompressedItems, data, 'Decompressed data does not match source data.');
    ...
    describe('encoding with combined encodings', function () {
        it('single item, raw, run length and diff', function (done) {
            var data = [{ something: { name: 'John', id: 1234 }, state: 'OK', balance: 2000.01 }];
            var definition = {
                properties: {
                    something: { encoding: 2 /* RAW */ },
                    state: { encoding: 0 /* RUNLENGTH */ },
                    balance: { encoding: 1 /* DIFF */, decimalDigits: 2 }
                }
            };
            var collection = new CompressedJsonCollection_1.default(definition, data);
            Should.equal(collection.items.length, data.length, 'Collection contains wrong number of items.');
        ...
        it('multiple items, raw, run length and diff', function (done) {
            var data = [
        ...
        it('many items, raw, run length and diff', function (done) {
            var data = [];
            ...
            var definition = {
                properties: {
                    something: { encoding: 2 /* RAW */ },
                    state: { encoding: 0 /* RUNLENGTH */ },
                    balance: { encoding: 1 /* DIFF */, decimalDigits: 4 }
                }
            };
            var collection = new CompressedJsonCollection_1.default(definition, data);
            Should.equal(collection.items.length, data.length, 'Collection contains wrong number of items.');
    ...
    describe('encoding with combined encodings, add items', function () {
        it('empty collection, add one item.', function (done) {
            var data = [];
            var definition = {
                properties: {
                    something: { encoding: 2 /* RAW */ },
        ...
        it('single item, add one item. Raw, run length and diff.', function (done) {
            var data = [{ something: { name: 'John', id: 1234 }, state: 'OK', balance: 2000.01 }];
            var definition = {
                properties: {
                    something: { encoding: 2 /* RAW */ },
        ...
        it('single item, add multiple items. Raw, run length and diff.', function (done) {
            var data = [{ something: { name: 'John', id: 1234 }, state: 'OK', balance: 2000.01 }];
            var definition = {
                properties: {
                    something: { encoding: 2 /* RAW */ },
    ...
    describe('encoding with combined encodings, remove items', function () {
        it('empty collection, add one item, remove one item by reference', function (done) {
            var definition = {
                properties: {
                    something: { encoding: 2 /* RAW */ },
                    state: { encoding: 0 /* RUNLENGTH */ },
            ...
            var collection = new CompressedJsonCollection_1.default(definition);
            var item = { something: { name: 'Jane', id: 4321 }, state: 'OK', balance: 2000.02 };
            collection.add(item);
        ...
        it('empty collection, add one item, remove one item by index', function (done) {
            var definition = {
                properties: {
                    something: { encoding: 2 /* RAW */ },
                    state: { encoding: 0 /* RUNLENGTH */ },
        ...
        it('sorted collection with initial items, add one item, remove one item by reference', function (done) {
            var item = { val: 3, rl: 'OK' };
            var data = [{ val: 0, rl: 'OK' }, { val: 1, rl: 'OK' }, { val: 2, rl: 'OK' }, { val: 4, rl: 'OK' }];
            var sort = function (a, b) { return a.val < b.val ? -1 : a.val > b.val ? 1 : 0; };
            var definition = {
                sort: sort,
                properties: {
                    val: { encoding: 1 /* DIFF */, decimalDigits: 0 },
                    rl: { encoding: 0 /* RUNLENGTH */ },
                }
            };
            var collection = new CompressedJsonCollection_1.default(definition, data);
            collection.add(item);
        ...
        it('sorted collection with initial items, add one item, remove one item by index', function (done) {
            var item = { val: 3, rl: 'OK' };
            var data = [{ val: 0, rl: 'OK' }, { val: 1, rl: 'OK' }, { val: 2, rl: 'OK' }, { val: 4, rl: 'OK' }];
            var sort = function (a, b) { return a.val < b.val ? -1 : a.val > b.val ? 1 : 0; };
            var definition = {
        ...
        it('sorted collection with initial items, remove two items (not in sequence)', function (done) {
            var item1 = { val: 1, rl: 'OK' };
            var item2 = { val: 3, rl: 'OK' };
            var data = [{ val: 0, rl: 'OK' }, item1, { val: 2, rl: 'OK' }, item2, { val: 4, rl: 'OK' }];
            var sort = function (a, b) { return a.val < b.val ? -1 : a.val > b.val ? 1 : 0; };
            var definition = {
                sort: sort,
                properties: {
            ...
            var decompressedItems = CompressedJsonCollection_1.default.decompress(collection.compressedJson);
            Should.deepEqual(decompressedItems, data.filter(function (item) { return item != item1 && item != item2; }), 'Decompressed data does not match source data.');
            done();
        });
        it('sorted collection with initial items, remove two items (in sequence)', function (done) {
            var item1 = { val: 1, rl: 'OK' };
            var item2 = { val: 2, rl: 'OK' };
            var data = [{ val: 0, rl: 'OK' }, item1, item2, { val: 3, rl: 'OK' }, { val: 4, rl: 'OK' }];
            var sort = function (a, b) { return a.val < b.val ? -1 : a.val > b.val ? 1 : 0; };
            var definition = {
                sort: sort,
                properties: {
        ...
        it('sorted collection with initial items, remove one item by index with decres object count (to 1) in object rl case', function (done) {
            var item1 = { val: 1, rl: 'OK' };
            var data = [{ val: 0, rl: 'OK' }, item1, { val: 6, rl: 'BAD' }, { val: 7, rl: 'OK' }];
            var sort = function (a, b) { return a.val < b.val ? -1 : a.val > b.val ? 1 : 0; };
            var definition = {
                sort: sort,
                properties: {
            ...
            //console.log('rl compression: ', collection.compressedJson.runlengthData);
            var decompressedItems = CompressedJsonCollection_1.default.decompress(collection.compressedJson);
            Should.deepEqual(decompressedItems, data.filter(function (item) { return item != item1; }), 'Decompressed data does not match source data.');
            done();
        });
        it('sorted collection with initial items, remove two items (in sequence) by start and end index,  decres object count (to 0) in object rl case', function (done) {
            var item0 = { val: 0, rl: 'OK' };
            var item1 = { val: 1, rl: 'OK' };
            var data = [item0, item1, { val: 6, rl: 'BAD' }, { val: 7, rl: 'OK' }];
            var sort = function (a, b) { return a.val < b.val ? -1 : a.val > b.val ? 1 : 0; };
            var definition = {
                sort: sort,
                properties: {
            ...
            //console.log('rl compression: ', collection.compressedJson.runlengthData);
            var decompressedItems = CompressedJsonCollection_1.default.decompress(collection.compressedJson);
            Should.deepEqual(decompressedItems, data.filter(function (item) { return item != item1 && item != item0; }), 'Decompressed data does not match source data.');
            done();
        });
        it('sorted collection with initial items, remove two items (in sequence) by start and end index, removes one item (none object) and decres object count (to 2) in object rl case', function (done) {
            var item0 = { val: 0, rl: 'BAD' };
            var item1 = { val: 1, rl: 'OK' };
            var item2 = { val: 2, rl: 'OK' };
            var data = [item0, item1, item2, { val: 3, rl: 'OK' }, { val: 6, rl: 'BAD' }, { val: 7, rl: 'OK' }];
            var sort = function (a, b) { return a.val < b.val ? -1 : a.val > b.val ? 1 : 0; };
            var definition = {
                sort: sort,
                properties: {
            ...
            //console.log('rl compression: ', collection.compressedJson.runlengthData);
            var decompressedItems = CompressedJsonCollection_1.default.decompress(collection.compressedJson);
            Should.deepEqual(decompressedItems, data.filter(function (item) { return item != item0 && item != item1; }), 'Decompressed data does not match source data.');
            done();
        });
        it('sorted collection with initial items, remove two items (in sequence) by start and end index, removes one item (none object) and decrese object count (to 1) in object rl case', function (done) {
            var item0 = { val: 0, rl: 'BAD' };
            var item1 = { val: 1, rl: 'OK' };
            var item2 = { val: 2, rl: 'OK' };
            var data = [item0, item1, item2, { val: 6, rl: 'BAD' }, { val: 7, rl: 'OK' }];
            var sort = function (a, b) { return a.val < b.val ? -1 : a.val > b.val ? 1 : 0; };
            var definition = {
                sort: sort,
                properties: {
        ...
        it('sorted collection with initial items, remove two items (in sequence) by start and end index, decreses two object counts to 1', function (done) {
            var item1 = { val: 1, rl: 'BAD' };
            var item2 = { val: 2, rl: 'OK' };
            var data = [{ val: 0, rl: 'BAD' }, item1, item2, { val: 3, rl: 'OK' }];
            var sort = function (a, b) { return a.val < b.val ? -1 : a.val > b.val ? 1 : 0; };
            var definition = {
                sort: sort,
                properties: {
        ...
        it('sorted collection with initial items, remove two items (in sequence) by start and end index, decreses one object count to 1 and removes one item', function (done) {
            var item1 = { val: 1, rl: 'BAD' };
            var item2 = { val: 2, rl: 'OK' };
            var data = [{ val: 0, rl: 'BAD' }, item1, item2];
            var sort = function (a, b) { return a.val < b.val ? -1 : a.val > b.val ? 1 : 0; };
            var definition = {
                sort: sort,
                properties: {
        ...
        it('sorted collection with initial items, remove three items (in sequence) by start and end index, decreses two object counts to 1 and removes one item', function (done) {
            var item1 = { val: 1, rl: 'BAD' };
            var item2 = { val: 2, rl: 'OK' };
            var item3 = { val: 3, rl: 'NOT OK' };
            ;
            var data = [{ val: 0, rl: 'BAD' }, item1, item2, item3, { val: 4, rl: 'NOT OK' }];
            var sort = function (a, b) { return a.val < b.val ? -1 : a.val > b.val ? 1 : 0; };
            var definition = {
                sort: sort,
                properties: {
            ...
            //console.log('rl compression: ', collection.compressedJson.runlengthData);
            var decompressedItems = CompressedJsonCollection_1.default.decompress(collection.compressedJson);
            Should.deepEqual(decompressedItems, data.filter(function (item) { return item != item1 && item != item2 && item != item3; }), 'Decompressed data does not match source data.');
            done();
        });
        it('sorted collection with initial items, remove items (in sequence) by index with varying cases in rl encoding', function (done) {
            var item1 = { val: 1, rl: 'OK' };
            var item2 = { val: 2, rl: 'NOT OK' };
            var item3 = { val: 3, rl: 'NOT OK' };
            var item4 = { val: 4, rl: 'OK' };
            var item5 = { val: 5, rl: 'BAD' };
            var data = [{ val: 0, rl: 'OK' }, item1, item2, item3, item4, item5, { val: 6, rl: 'BAD' }, { val: 7, rl: 'OK' }];
            var sort = function (a, b) { return a.val < b.val ? -1 : a.val > b.val ? 1 : 0; };
            var definition = {
                sort: sort,
                properties: {
            ...
            //console.log('rl compression: ', collection.compressedJson.runlengthData);
            var decompressedItems = CompressedJsonCollection_1.default.decompress(collection.compressedJson);
            Should.deepEqual(decompressedItems, data.filter(function (item) { return item != item1 && item != item2 && item != item3 && item != item4 && item != item5; }), 'Decompressed data does not match source data.');
            done();
        });
    });
    describe('events', function () {
        ...
        it('removed', function (done) {
            ...
            var definition = { properties: { id: { encoding: 1 /* DIFF */ } } };
            var collection = new CompressedJsonCollection_1.default(definition, data);
            collection.on('removed', function (item) {
                Should.deepEqual(data[0], item);
                done();
            });
            collection.remove(data[0]);
        });
        it('removed (by index)', function (done) {
            var data = [{ id: 0 }, { id: 1 }];
            var definition = { properties: { id: { encoding: 1 /* DIFF */ } } };
            var collection = new CompressedJsonCollection_1.default(definition, data);
            var count = 0;
...
describe('CompressedCollection inserts with sort', function () {
    describe('encoding with sorting, basics', function () {
        it('empty collection, add one item.', function (done) {
            var sort = function (a, b) { return a.val < b.val ? -1 : a.val > b.val ? 1 : 0; };
            var data = [];
            var definition = {
        ...
        it('one items in collection, add one (greater) item.', function (done) {
            var sort = function (a, b) { return a.val < b.val ? -1 : a.val > b.val ? 1 : 0; };
            var data = [{ val: 0, rl: 'OK' }];
            var definition = {
                sort: sort,
                properties: {
                    val: { encoding: 1 /* DIFF */, decimalDigits: 0 },
        ...
        it('one items in collection, add one (lesser) item.', function (done) {
            var sort = function (a, b) { return a.val < b.val ? -1 : a.val > b.val ? 1 : 0; };
            var data = [{ val: 1, rl: 'OK' }];
            var definition = {
                sort: sort,
                properties: {
                    val: { encoding: 1 /* DIFF */, decimalDigits: 0 },
        ...
        it('four items in collection, add one item to the "center" of the collection.', function (done) {
            var sort = function (a, b) { return a.val < b.val ? -1 : a.val > b.val ? 1 : 0; };
            var data = [{ val: 0, rl: 'OK' }, { val: 1, rl: 'OK' }, { val: 3, rl: 'OK' }, { val: 4, rl: 'OK' }];
            var definition = {
                sort: sort,
                properties: {
                    val: { encoding: 1 /* DIFF */, decimalDigits: 0 },
        ...
        it('four items in collection, add one item to the second position of the collection.', function (done) {
            var sort = function (a, b) { return a.val < b.val ? -1 : a.val > b.val ? 1 : 0; };
            var data = [{ val: 0, rl: 'OK' }, { val: 2, rl: 'OK' }, { val: 3, rl: 'OK' }, { val: 4, rl: 'OK' }];
            var definition = {
                sort: sort,
                properties: {
                    val: { encoding: 1 /* DIFF */, decimalDigits: 0 },
        ...
        it('four items in collection, add one item to the second to last position of the collection.', function (done) {
            var sort = function (a, b) { return a.val < b.val ? -1 : a.val > b.val ? 1 : 0; };
            var data = [{ val: 0, rl: 'OK' }, { val: 1, rl: 'OK' }, { val: 2, rl: 'OK' }, { val: 4, rl: 'OK' }];
            var definition = {
                sort: sort,
    ...
    describe('encoding with sorting, runlength testing', function () {
        it('four items in collection, add one item to the "center" of the collection (with unmatching RL item for that location).', function (done) {
            var sort = function (a, b) { return a.val < b.val ? -1 : a.val > b.val ? 1 : 0; };
            var data = [{ val: 0, rl: 'OK' }, { val: 1, rl: 'OK' }, { val: 3, rl: 'OK' }, { val: 4, rl: 'OK' }];
            var definition = {
                sort: sort,
        ...
        it('four items in collection, add one item to the "center" of the collection (with distinct rl items, match is NOT object).', function (done) {
            var sort = function (a, b) { return a.val < b.val ? -1 : a.val > b.val ? 1 : 0; };
            var data = [{ val: 0, rl: 'OK1' }, { val: 1, rl: 'OK2' }, { val: 3, rl: 'OK3' }, { val: 4, rl: 'OK4' }];
            var definition = {
                sort: sort,
                properties: {
                    val: { encoding: 1 /* DIFF */, decimalDigits: 0 },
        ...
        it('four items in collection, add one item to the "center" of the collection (matcing rl item IS object).', function (done) {
            var sort = function (a, b) { return a.val < b.val ? -1 : a.val > b.val ? 1 : 0; };
            var data = [{ val: 0, rl: 'OK' }, { val: 1, rl: 'OK' }, { val: 3, rl: 'OK3' }, { val: 4, rl: 'OK4' }];
            var definition = {
                sort: sort,
                properties: {
                    val: { encoding: 1 /* DIFF */, decimalDigits: 0 },
        ...
        it('four items in collection, add one item to the "center" of the collection (matcing rl item IS object splite in to object and string).', function (done) {
            var sort = function (a, b) { return a.val < b.val ? -1 : a.val > b.val ? 1 : 0; };
            var data = [{ val: 0, rl: 'OK' }, { val: 1, rl: 'OK' }, { val: 3, rl: 'OK' }, { val: 4, rl: 'OK4' }];
            var definition = {
                sort: sort,
                properties: {
                    val: { encoding: 1 /* DIFF */, decimalDigits: 0 },
        ...
        it('four items in collection, add one item to the "center" of the collection (matcing rl item IS object splite in to string and object).', function (done) {
            var sort = function (a, b) { return a.val < b.val ? -1 : a.val > b.val ? 1 : 0; };
            var data = [{ val: 0, rl: 'OK1' }, { val: 1, rl: 'OK' }, { val: 3, rl: 'OK' }, { val: 4, rl: 'OK' }];
            var definition = {
                sort: sort,
                properties: {
                    val: { encoding: 1 /* DIFF */, decimalDigits: 0 },
...
describe('CompressedCollection querying', function () {
    it('querying collection, test one', function (done) {
        var sort = function (a, b) { return a.val < b.val ? -1 : a.val > b.val ? 1 : 0; };
        var data = [{ val: 0, rl: 'OK' }, { val: 1, rl: 'OK' }, { val: 2, rl: 'ARCHIVED' }, { val: 3, rl: 'OK' }, { val: 4, rl: 'OK' }];
        var definition = {
            sort: sort,
            properties: {
                val: { encoding: 1 /* DIFF */, decimalDigits: 0 },
        ...
        var collection = new CompressedJsonCollection_1.default(definition, data);
        Should.equal(collection.items.length, data.length, 'Collection contains wrong number of items.');
        var result = collection.query(function (item) { return item.rl === 'OK'; });
        var decompressedItems = CompressedJsonCollection_1.default.decompress(result);
        Should.deepEqual(decompressedItems, data.filter(function (d) { return d.rl === 'OK'; }), 'Decompressed data does not match source data.');
        done();
    });
    it('querying collection, test two', function (done) {
        var sort = function (a, b) { return a.val < b.val ? -1 : a.val > b.val ? 1 : 0; };
        var data = [{ val: 0, rl: 'OK' }, { val: 1, rl: 'OK' }, { val: 2, rl: 'ARCHIVED' }, { val: 3, rl: 'OK' }, { val: 4, rl: 'OK' }];
        var definition = {
            sort: sort,
        ...
        var collection = new CompressedJsonCollection_1.default(definition, data);
        Should.equal(collection.items.length, data.length, 'Collection contains wrong number of items.');
        var result = collection.query(function (item) { return item.val > 0 && item.val < 4; });
        var decompressedItems = CompressedJsonCollection_1.default.decompress(result);
        Should.deepEqual(decompressedItems, data.filter(function (d) { return d.val > 0 && d.val < 4; }), 'Decompressed data does not match source data.');
        done();
    });
    it('querying collection, test three', function (done) {
        var sort = function (a, b) { return a.val < b.val ? -1 : a.val > b.val ? 1 : 0; };
        var data = [{ val: 0, rl: 'OK' }, { val: 1, rl: 'OK' }, { val: 2, rl: 'ARCHIVED' }, { val: 3, rl: 'OK' }, { val: 4, rl: 'OK' }];
        var definition = {
            sort: sort,
        ...
        var collection = new CompressedJsonCollection_1.default(definition, data);
        Should.equal(collection.items.length, data.length, 'Collection contains wrong number of items.');
        var result = collection.query(function (item) { return item.val < 1 && item.val > 3; });
        var decompressedItems = CompressedJsonCollection_1.default.decompress(result);
        Should.deepEqual(decompressedItems, data.filter(function (d) { return d.val < 1 && d.val > 3; }), 'Decompressed data does not match source data.');
        done();
    });
    it('querying collection, test four', function (done) {
        var sort = function (a, b) { return a.val < b.val ? -1 : a.val > b.val ? 1 : 0; };
        var data = [{ val: 0, rl: 'OK' }, { val: 1, rl: 'OK' }, { val: 2, rl: 'ARCHIVED' }, { val: 3, rl: 'OK' }, { val: 4, rl: 'OK' }];
        var definition = {
            sort: sort,
        ...
        var collection = new CompressedJsonCollection_1.default(definition, data);
        Should.equal(collection.items.length, data.length, 'Collection contains wrong number of items.');
        var result = collection.query(function (item) { return item.val % 2 == 0; });
        var decompressedItems = CompressedJsonCollection_1.default.decompress(result);
        Should.deepEqual(decompressedItems, data.filter(function (d) { return d.val % 2 == 0; }), 'Decompressed data does not match source data.');
        done();
    });
    it('querying collection, test five', function (done) {
        var sort = function (a, b) { return a.val < b.val ? -1 : a.val > b.val ? 1 : 0; };
        var data = [{ val: 0, rl: 'OK' }, { val: 1, rl: 'OK' }, { val: 2, rl: 'ARCHIVED' }, { val: 3, rl: 'OK' }, { val: 4, rl: 'OK' }];
        var definition = {
            sort: sort,
        ...
        var collection = new CompressedJsonCollection_1.default(definition, data);
        Should.equal(collection.items.length, data.length, 'Collection contains wrong number of items.');
        var result = collection.query(function (item) { return item.val % 2 == 1; });
        var decompressedItems = CompressedJsonCollection_1.default.decompress(result);
        Should.deepEqual(decompressedItems, data.filter(function (d) { return d.val % 2 == 1; }), 'Decompressed data does not match source data.');
        done();
    });
    it('querying collection, test six', function (done) {
        var sort = function (a, b) { return a.val < b.val ? -1 : a.val > b.val ? 1 : 0; };
        var data = [{ val: 0, rl: 'OK' }, { val: 1, rl: 'OK' }, { val: 2, rl: 'ARCHIVED' }, { val: 3, rl: 'OK' }, { val: 4, rl: 'OK' }];
        var definition = {
            sort: sort,
        ...
        var collection = new CompressedJsonCollection_1.default(definition, data);
        Should.equal(collection.items.length, data.length, 'Collection contains wrong number of items.');
        var result = collection.query(function (item) { return item.val % 2 == 1 || item.rl == 'ARCHIVED'; });
        var decompressedItems = CompressedJsonCollection_1.default.decompress(result);
        Should.deepEqual(decompressedItems, data.filter(function (d) { return d.val % 2 == 1 || d.rl == 'ARCHIVED'; }), 'Decompressed data does not match source data.');
        done();
    });
    it('querying collection, test six', function (done) {
        var sort = function (a, b) { return a.val < b.val ? -1 : a.val > b.val ? 1 : 0; };
...
describe('CompressedCollection inserts with insertionHandler', function () {
    describe('basic filter', function () {
        describe('unsorted inserting', function () {
            it('empty collection with basic filtertype, add one item that should not be filtered', function (done) {
                var data = [];
                var definition = {
                    insertionHandler: function (item) { return item.val < 4; },
                    properties: {
                        val: { encoding: 1 /* DIFF */, decimalDigits: 0 },
                        rl: { encoding: 0 /* RUNLENGTH */ }
                    }
            ...
            it('empty collection with basic filtertype, add one item that should be filtered', function (done) {
                var data = [];
                var definition = {
                    insertionHandler: function (item) { return item.val > 4; },
                    properties: {
            ...
            it('initiating collection filtered with basic filtertype with data, one items should be filtered away', function (done) {
                var data = [{ val: 0, rl: 'OK' }, { val: 1, rl: 'OK' }, { val: 2, rl: 'OK' }, { val: 3, rl: 'OK' }, { val: 4, rl: 'OK' }];
                var definition = {
                    insertionHandler: function (item) { return item.val < 4; },
                    properties: {
                        val: { encoding: 1 /* DIFF */, decimalDigits: 0 },
                ...
                var collection = new CompressedJsonCollection_1.default(definition, data);
                data = data.filter(function (item) { return item.val < 4; });
                Should.equal(collection.items.length, data.length, 'Collection contains wrong number of items.');
                var decompressedItems = CompressedJsonCollection_1.default.decompress(collection.compressedJson);
                Should.deepEqual(decompressedItems, data, 'Decompressed data does not match source data.');
                done();
            });
            it('initiating collection filtered with basic filtertype with data, all items should be filtered away', function (done) {
                var data = [{ val: 0, rl: 'OK' }, { val: 1, rl: 'OK' }, { val: 2, rl: 'OK' }, { val: 3, rl: 'OK' }, { val: 4, rl: 'OK' }];
                var definition = {
                    insertionHandler: function (item) { return item.val > 4; },
                    properties: {
            ...
            it('initiating collection filtered with basic filtertype with data, some items should be filtered away', function (done) {
                var data = [{ val: 0, rl: 'OK' }, { val: 1, rl: 'OK' }, { val: 2, rl: 'OK' }, { val: 3, rl: 'OK' }, { val: 4, rl: 'OK' }];
                var definition = {
                    insertionHandler: function (item) { return item.val > 1; },
                    properties: {
                        val: { encoding: 1 /* DIFF */, decimalDigits: 0 },
                        rl: { encoding: 0 /* RUNLENGTH */ }
                    }
                ...
                var collection = new CompressedJsonCollection_1.default(definition, data);
                data = data.filter(function (item) { return item.val > 1; });
                Should.equal(collection.items.length, data.length, 'Collection contains wrong number of items.');
                var decompressedItems = CompressedJsonCollection_1.default.decompress(collection.compressedJson);
                Should.deepEqual(decompressedItems, data, 'Decompressed data does not match source data.');
                done();
        ...
        describe('sorted inserting', function () {
            it('empty collection with basic filtertype, add one item that should not be filtered', function (done) {
                var sort = function (a, b) { return a.val < b.val ? -1 : a.val > b.val ? 1 : 0; };
                var data = [];
                var definition = {
            ...
            it('initiating collection filtered with basic filtertype with data, one items should be filtered away', function (done) {
                var sort = function (a, b) { return a.val < b.val ? -1 : a.val > b.val ? 1 : 0; };
                var data = [{ val: 1, rl: 'OK' }, { val: 0, rl: 'OK' }, { val: 3, rl: 'OK' }, { val: 2, rl: 'OK' }, { val: 4, rl: 'OK' }];
                var definition = {
                    sort: sort,
                    insertionHandler: function (item) { return item.val < 4; },
                    properties: {
                ...
                var collection = new CompressedJsonCollection_1.default(definition, data);
                data = data.sort(sort).filter(function (item) { return item.val < 4; });
                Should.equal(collection.items.length, data.length, 'Collection contains wrong number of items.');
                var decompressedItems = CompressedJsonCollection_1.default.decompress(collection.compressedJson);
                Should.deepEqual(decompressedItems, data, 'Decompressed data does not match source data.');
                done();
    ...
    describe('advanced filter', function () {
        describe('unsorted inserting', function () {
            it('empty collection with basic filtertype, add one item that should not be filtered', function (done) {
                var data = [];
                var definition = {
                    insertionHandler: new (/** @class */ (function () {
                        function class_1() {
                        }
                        class_1.prototype.insert = function (items, collection) {
                            return items.filter(function (item) { return item.val < 4; });
                        };
                        ;
                        return class_1;
                    }()))(),
            ...
            it('empty collection with basic filtertype, add one item that should be filtered', function (done) {
                ...
                var definition = {
                    insertionHandler: new (/** @class */ (function () {
                        ...
                        class_2.prototype.insert = function (items, collection) {
                            return items.filter(function (item) { return item.val > 4; });
                        };
                        ;
                        return class_2;
            ...
            it('initiating collection filtered with basic filtertype with data, one items should be filtered away', function (done) {
                ...
                var definition = {
                    insertionHandler: new (/** @class */ (function () {
                        ...
                        class_3.prototype.insert = function (items, collection) {
                            return items.filter(function (item) { return item.val < 4; });
                        };
                        ;
                        return class_3;
            ...
            it('initiating collection filtered with basic filtertype with data, all items should be filtered away', function (done) {
                ...
                var definition = {
                    insertionHandler: new (/** @class */ (function () {
                        ...
                        class_4.prototype.insert = function (items, collection) {
                            return items.filter(function (item) { return item.val > 4; });
                        };
                        ;
                        return class_4;
            ...
            it('initiating collection filtered with basic filtertype with data, some items should be filtered away', function (done) {
                ...
                var definition = {
                    insertionHandler: new (/** @class */ (function () {
                        ...
                        class_5.prototype.insert = function (items, collection) {
                            return items.filter(function (item) { return item.val > 1; });
                        };
                        ;
                        return class_5;
                    }()))(),
            ...
            it('empty collection with basic filtertype, add one item that should not be filtered', function (done) {
                ...
                var definition = {
                    ...
                    insertionHandler: new (/** @class */ (function () {
                        ...
                        class_6.prototype.insert = function (items, collection) {
                            return items.filter(function (item) { return item.val < 4; });
                        };
                        ;
                        return class_6;
            ...
            it('empty collection with basic filtertype, add one item that should be filtered', function (done) {
                ...
                var definition = {
                    ...
                    insertionHandler: new (/** @class */ (function () {
                        ...
                        class_7.prototype.insert = function (items, collection) {
                            return items.filter(function (item) { return item.val > 4; });
                        };
                        ;
                        return class_7;
            ...
            it('initiating collection filtered with basic filtertype with data, one items should be filtered away', function (done) {
                ...
                var definition = {
                    ...
                    insertionHandler: new (/** @class */ (function () {
                        ...
                        class_8.prototype.insert = function (items, collection) {
                            return items.filter(function (item) { return item.val < 4; });
                        };
                        ;
                        return class_8;
            ...
            it('initiating collection filtered with basic filtertype with data, all items should be filtered away', function (done) {
                ...
                var definition = {
                    ...
                    insertionHandler: new (/** @class */ (function () {
                        ...
                        class_9.prototype.insert = function (items, collection) {
                            return items.filter(function (item) { return item.val > 4; });
                        };
                        ;
                        return class_9;
            ...
            it('initiating collection filtered with basic filtertype with data, some items should be filtered away', function (done) {
                ...
                var definition = {
                    ...
                    insertionHandler: new (/** @class */ (function () {
                        ...
                        class_10.prototype.insert = function (items, collection) {
                            return items.filter(function (item) { return item.val > 1; });
                        };
                        ;
                        return class_10;
        ...
        describe('advanced usage tests', function () {
            it('buffered insertion handler', function (done) {
                var ItemBuffer = /** @class */ (function () {
                    function class_11() {
                        this.buffer = [];
                    }
                    class_11.prototype.insert = function (items, collection) {
                        var _this = this;
                        if (this.buffer.length > 4)
                            this.buffer = [];
                        items.forEach(function (item) { return _this.buffer.push(item); });
                        if (this.buffer.length > 4)
                            return this.buffer;
                        else
                            return [];
                ...
                var definition = {
                    ...
                    properties: {
                        val: { encoding: 1 /* DIFF */, decimalDigits: 0 },
                        rl: { encoding: 0 /* RUNLENGTH */ },
                        insertIdx: { encoding: 2 /* RAW */ }
                    }
                };
                var collection = new CompressedJsonCollection_1.default(definition, data);
                Should.equal(collection.items.length, data.length, 'Collection contains wrong number of items.');
                var vals = [34, 6, 12, 3, 12, 6, 32, 1, 8, 12];
                var _loop_1 = function (idx) {
                    var item = { val: vals[idx % 10], rl: idx % 3 == 0 ? 'OK' : 'NOT OK', insertIdx: idx };
                    collection.add(item);
                    data.push(item);
                    if ((idx + 1) % 5 == 0) {
                        data.sort(sort);
                        Should.equal(collection.items.length, data.length, 'Collection contains wrong number of items.');
                        var decompressedItems_1 = CompressedJsonCollection_1.default.decompress(collection.compressedJson);
                        Should.deepEqual(decompressedItems_1.map(function (i) { return i.val; }), data.map(function (i) { return i.val; }), 'Decompressed data does not match source data.');
                        data.map(function (i) { return ({ src: i, res: decompressedItems_1.filter(function (di) { return di.insertIdx === i.insertIdx; })[0] }); })
            ...
            it('handler that alters collection', function (done) {
                var InsertionHandler = /** @class */ (function () {
                    function class_12() {
                        this.buffer = [];
                        this.isAlteringInternal = false;
                    }
                    class_12.prototype.insert = function (items, collection) {
                        var _this = this;
                        if (this.isAlteringInternal)
                            return items;
                        if (this.buffer.length + items.length > 4) {
                            collection.remove(this.buffer);
                            var lastItemExceptBufferToBeAdded = collection.items[collection.items.length - 1];
                            collection.remove(lastItemExceptBufferToBeAdded);
                            var allItems = (lastItemExceptBufferToBeAdded != null ? [lastItemExceptBufferToBeAdded] : []).concat(this.buffer.concat(items).sort(collection.definition.sort));
                            var itemsToAdd = allItems.filter(function (item, idx) { return idx == 0 || (idx + 1) % 5 == 0; });
            ...
            it('real advanced gps usecase (time baser insert filtering)', function (done) {
                var InsertionHandler = /** @class */ (function () {
                    ...
                    class_13.prototype.insert = function (items, collection) {
                        ...
                        if (this.bufferMax.getTime() - this.bufferMin.getTime() > 3600000 * 2) {
                            collection.remove(this.buffer);
                            var lastAddedItem = collection.items[collection.items.length - 1];
                            collection.remove(lastAddedItem);
                            var allCandidates = (lastAddedItem != null ? [lastAddedItem] : []).concat(this.buffer);
                ...
                var sort = function (a, b) { return a.time < b.time ? -1 : a.time > b.time ? 1 : 0; };
                var data = gpsTestData.slice(1, 10);
                var definition = {
                    sort: sort,
                    insertionHandler: new InsertionHandler(),
                    properties: {
                        time: { encoding: 1 /* DIFF */, type: 'date', factor: 0.0001 },
                        lat: { encoding: 1 /* DIFF */, decimalDigits: 4 },
                        lon: { encoding: 1 /* DIFF */, decimalDigits: 4 }
                    }
                };
                new CompressedJsonCollection_1.default(definition, data);
